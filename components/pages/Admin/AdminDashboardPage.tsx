...
const AdminDashboardPage: React.FC = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      if (!data) {
        console.warn('No submissions data returned');
        throw new Error('No submissions data returned');
      }
      console.log('Raw submissions data:', data); // Debug log
      const formattedSubmissions = (data || []).map((submission: any, index: number) => ({
        id: submission.id ? submission.id.toString() : `submission-${index}`,
        ...
        <div className="bg-[#18181b] rounded-lg shadow-sm border border-[#23231f] p-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
            ...
            <div>
              ...
              >
                {months.map((m, i) => <option key={`month-${i}`} value={m}>{m}</option>)}
              </select>
            </div>
            {/* Status Filter */}
            <div>
              ...
              >
                {statuses.map((s, i) => <option key={`status-${i}`} value={s}>{s}</option>)}
              </select>
            </div>
            {/* Search Filter */}
            <div>
                    ...
                    <React.Fragment key={submission.id}>
                      <tr className="hover:bg-[#23231f] cursor-pointer" onClick={() => setSelectedSubmission(selectedSubmission?.id === submission.id ? null : submission)}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <Button variant="outline" size="sm" onClick={(e: React.MouseEvent) => { e.stopPropagation(); window.open(submission.videoUrl, '_blank'); }}>
                            <Eye className="h-4 w-4 mr-1" />
                            {t('submissions.actions.viewVideo')}
                          </Button>
                          <Button variant="secondary" size="sm" className="ml-2" onClick={(e: React.MouseEvent) => { e.stopPropagation(); setSelectedSubmission(selectedSubmission?.id === submission.id ? null : submission); }}>
